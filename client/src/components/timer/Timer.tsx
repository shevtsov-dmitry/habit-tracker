...
class ClockIds {
    ...
    constructor(hourId: string, minuteId: string, secondId: string | null) {
        this.hourId = hourId
...
const Timer = ({
    ...
    const clockIdsRef = useRef<ClockIds>(new ClockIds('', '', null))
    ...
    function parseOffset(offsetStr: string): number | null {
        const match = offsetStr.match(/^([+-])(\d{2}):(\d{2})$/)
        if (!match) {
            console.error(
                'Wrong time offset format. Should match example +00:00.'
            )
            return null
        }
    ...
    function initClock() {
        ...
        const ids = new ClockIds(
            'hour-' + getRandomId(),
            'minute-' + getRandomId(),
            useSeconds ? 'second-' + getRandomId() : null
        )

        clockIdsRef.current = ids


        const hour = createClockElement(
            ids.hourId,
            '#333333',
            '3%',
            '25%',
        ...
        const minute = createClockElement(
            ids.minuteId,
            '#333333',
            '2%',
            '30%',
    ...
    function tick() {
        const now = new Date(
            Date.now() +
            (new Date().getTimezoneOffset() + timeOffsetNumber) * 60 * 1000
        )
        const h = now.getHours()
        const m = now.getMinutes()
        const s = useSeconds ? now.getSeconds() : 0
        const hourDeg = h * 30 + m * 0.5 + s * 0.00833333333
        const minuteDeg = m * 6 + s * 0.1

        const {hourId, minuteId, secondId} = clockIdsRef.current

        const hourArrow = document.getElementById(hourId)
        hourArrow && (hourArrow.style.transform = `rotate(${hourDeg}deg)`)

        const minuteArrow = document.getElementById(minuteId)
        minuteArrow && (minuteArrow.style.transform = `rotate(${minuteDeg}deg)`)

        if (useSeconds) {
            const secondDeg = s * 6
            const secondsArrow = document.getElementById(secondId!)
            secondsArrow &&
    ...
    return (
        <div className="flex items-center justify-center h-16">
            <div
                className={
                    'w-96 h-96 bg-amber-500 rounded-lg shadow-md flex flex-col items-center justify-center space-y-4 p-8'
                }
            >
